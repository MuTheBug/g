"""Monthly-income heatmap for the wide-universe daily neutral momentum basket."""
...
SRC = ROOT / "tool" / "daily_wide_basket_monthly.csv"
OUT = ROOT / "tool" / "daily_wide_basket_monthly.png"
...
fig, ax = plt.subplots(figsize=(11, 0.6 * len(years) + 2.2))
...
ax.set_title(r"Monthly income (USD) on fixed \$60 — wide-universe daily NEUTRAL momentum basket, +\$3 TP"
             + "\n" + f"total \\${sum(allv):+.0f} over {len(allv)} months · "
             f"green {pos}/{len(allv)} ({100*pos/len(allv):.0f}%) · "
             f"avg \\${sum(allv)/len(allv):+.2f}/mo", fontsize=10)
